...
def fetch_data(ticker: str) -> dict | None:
    try:
        ...
        sma200_1mo = df["SMA200"].iloc[-22]

        tt = {
            "1. Price > SMA150 & SMA200":      price > sma150 and price > sma200,
            "2. SMA150 > SMA200":              sma150 > sma200,
            "3. SMA200 Trending Up (1mo)":     sma200 > sma200_1mo,
            "4. SMA50 > SMA150 & SMA200":      sma50 > sma150 and sma50 > sma200,
            "5. Price > SMA50":                price > sma50,
            "6. Price >= 30% Above 52w Low":   price >= low52 * 1.30,
            "7. Price Within 25% of 52w High": price >= high52 * 0.75,
            "8. RS Positive (vs SMA200)":      price > sma200,
        }

        # ── Stage Detection (Weinstein) ──────────────────────────
        slope20 = (df["SMA150"].iloc[-1] - df["SMA150"].iloc[-20]) / df["SMA150"].iloc[-20] * 100
        ...
        near_highs = price >= df.tail(60)["High"].max() * 0.90
        vol_dry    = vols[0] > vols[1]
        pivot = final10["High"].max()
        pct_to_pivot = (pivot - price) / price * 100
        vcp_score = min(100, 30*contractions + (15 if vol_dry else 0)
...
# CHART
# ═══════════════════════════════════════════════════════════════
def make_chart(d: dict, ticker: str, stop_price: float,
               target_2r: float, target_3r: float) -> go.Figure:
    df = d["df"].tail(180)
    ...
    rs_color = "#3fb950" if rs_line.iloc[-1] >= rs_line.tail(20).max() * 0.99 else "#ab47bc"
    fig.add_trace(go.Scatter(
...
# AI MENTOR
# ═══════════════════════════════════════════════════════════════
def claude_analysis(ticker: str, d: dict, stop: float,
                    t2r: float, t3r: float, shares: int) -> str:
    client = get_claude()
    prompt = f"""You are Mark Minervini's trading methodology expert and mentor.
Analyze this SEPA setup and give a concise, actionable assessment. Be direct and specific.

TICKER: {ticker} ({d['name']}) | Sector: {d['sector']}
Price: ${d['price']:.2f} | Market Cap: ${d['mktcap']/1e9:.1f}B

TREND TEMPLATE: {d['tt_score']}/8
{json.dumps(d['tt'], indent=2)}

STAGE ANALYSIS: {d['stage_label']}
- SMA150 slope (20d): {d['slope20']}%

VCP ANALYSIS (Score: {d['vcp_score']}/100):
- Contractions: {d['contractions']} | Tight area: {d['tight_rng']}% | Near highs: {d['near_highs']}
- Volume drying: {d['vol_dry']} | Confirmed VCP: {d['is_vcp']}
- Pivot: ${d['pivot']} | % to pivot: {d['pct_to_pivot']}%

RISK/REWARD:
- Entry: ${d['pivot']:.2f} | Stop: ${stop:.2f} | 2R: ${t2r:.2f} | 3R: ${t3r:.2f}
- Position: {shares} shares

FUNDAMENTALS:
- EPS Growth (Q): {d['eps_growth']*100:.1f}%
- Revenue Growth: {d['rev_growth']*100:.1f}%
- ROE: {d['roe']*100:.1f}%

Respond with:
1. SETUP VERDICT: (Actionable / Watch / Avoid - one line, blunt)
2. STRENGTHS: (3 bullets max)
3. WEAKNESSES / RISKS: (3 bullets max)
4. IDEAL ENTRY: (specific price action to wait for)
5. MENTOR NOTE: (one key Minervini insight for this exact setup)"""
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "Data: yfinance | AI: Claude<br>Not financial advice</p>",
        unsafe_allow_html=True
    )
...
with tab_single:
    ...
    if ticker_input:
        ...
        if d is None:
            ...
            with ex1:
                suggested_stop = d["df"]["Low"].tail(15).min()
                stop_price = st.number_input(
                    "Stop Loss ($)",
                    value=float(round(suggested_stop, 2)),
            ...
            if risk_per_share > 0:
                ...
                stop_price = d["df"]["Low"].tail(15).min()
    ...
    if st.button("Run Scan", type="primary", use_container_width=True):
        ...
        for i, t in enumerate(tickers):
            ...
            if d["price"] < min_price:
                continue
            if d["df"]["Volume"].tail(50).mean() < min_vol:
                continue
            if d["tt_score"] < min_tt:
                continue
            if d["vcp_score"] < min_vcp:
                continue
            if require_stage2 and d["stage"] != 2:
                continue
            if require_vcp and not d["is_vcp"]:
                continue
            ...
            rows.append({
                ...
                "Extension%": round(ext, 1),
                "Pivot":      d["pivot"],
                "% to Pivot": d["pct_to_pivot"],
