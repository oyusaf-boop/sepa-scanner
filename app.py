...
def get_sp500(): return ["AAPL","MSFT","NVDA","AVGO","ORCL","CRM","ACN","ADBE","CSCO","TXN"]

@st.cache_data(ttl=86400, show_spinner=False)
def get_nasdaq100(): return ["AAPL","MSFT","NVDA","AMZN","META","GOOGL","TSLA","AVGO","COST","NFLX"]
...
# ── Updated Barchart-Style Charting Engine ────────────────────
def make_chart(d, ticker, stop_price, target_2r, target_3r):
    ...
    # Institutional Moving Averages (Barchart Style)
    fig.add_trace(go.Scatter(x=df.index, y=df["SMA20"], name="MA (20, Close, 0)", 
                             line=dict(color="#d62728", width=1.5)), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["EMA21"], name="MAEXP (21, Close, 0)", 
                             line=dict(color="#7f7f7f", width=1.5)), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["SMA50"], name="MA (50, Close, 0)", 
                             line=dict(color="#1f77b4", width=1.5)), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["SMA200"], name="MA (200, Close, 0)", 
                             line=dict(color="#2ca02c", width=1.5)), row=1, col=1)
    
    # Risk Execution Lines (Subtle)
    fig.add_hline(y=d["pivot"], line=dict(color="#1f77b4", width=1, dash="dash"), row=1, col=1)
    fig.add_hline(y=stop_price, line=dict(color="#d62728", width=1, dash="dot"), row=1, col=1)
    
    # Volume Subplot
    colors = ["#2ca02c" if c >= o else "#d62728" for c, o in zip(df["Close"], df["Open"])]
    fig.add_trace(go.Bar(
        x=df.index, y=df["Volume"], marker_color=colors,
        opacity=0.8, name="VOL (OpenToClose)", showlegend=True
    ), row=2, col=1)
    
    # Barchart Light-Mode Layout Configuration
    fig.update_layout(
        height=700,
        plot_bgcolor="#ffffff",
        ...
        legend=dict(
            ...
            bgcolor="rgba(255,255,255,0.8)", font=dict(size=12, color="#000000"),
    ...
    # Y-Axes on the right, light gray grids
    fig.update_yaxes(side="right", showgrid=True, gridcolor="#f0f0f0", zeroline=False)
